...
sl_status_t sl_gatt_service_hall_get(float *field_strength, bool *alert, bool *tamper)
{
  ...
  if (SL_STATUS_OK != sc) {
    app_log_error("[E: %#04x] Hall sensor measurement failed\n", sc);
  }
  return sc;
}
#endif
...
sl_status_t sl_gatt_service_light_get(float *lux, float *uvi)
{
  ...
  if (SL_STATUS_OK != sc) {
    app_log_error("[E: %#04x] Light sensor measurement failed\n", sc);
  }
  return sc;
}
#endif
...
sl_status_t sl_gatt_service_rht_get(uint32_t *rh, int32_t *t)
{
  ...
  if (SL_STATUS_OK != sc) {
    app_log_error("[E: %#04x] RHT sensor measurement failed\n", sc);
  }
  return sc;
}
#endif
...
sl_status_t sl_gatt_service_imu_calibrate(void)
{
  ...
  app_log_info("IMU calibration status: %d\n", sc);
  return sc;
}

void sl_gatt_service_imu_enable(bool enable)
{
  app_log_info("IMU %sable\n", enable ? "en" : "dis");
...
sl_status_t sl_gatt_service_pressure_get(float *pressure)
{
  ...
  if (SL_STATUS_OK != sc) {
    app_log_error("[E: %#04x] Pressure sensor measurement failed\n", sc);
  }
  return sc;
}
#endif
...
sl_status_t sl_gatt_service_gas_get(uint16_t *eco2, uint16_t *tvoc)
{
  ...
  if (!sl_power_supply_is_low_power()) {
    ...
    if ((SL_STATUS_OK != sc) && (SL_STATUS_NOT_READY != sc)) {
      app_log_error("[E: %#04x] Air quality sensor measurement failed\n", sc);
    }
  }
  return sc;
}
...
sl_status_t sl_gatt_service_sound_get(float *sound_level)
{
  ...
  if (SL_STATUS_OK != sc) {
    app_log_error("[E: %#04x] Sound level measurement failed\n", sc);
  }
  return sc;
}
#endif
